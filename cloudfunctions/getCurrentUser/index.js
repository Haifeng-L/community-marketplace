...
exports.main = async () => {
  ...
  if (typeof user.wechatNickname !== 'string') patch.wechatNickname = data.wechatNickname
  if (typeof user.avatarUrl !== 'string') patch.avatarUrl = data.avatarUrl
  if (typeof user.wechatAvatarUrl !== 'string') patch.wechatAvatarUrl = data.wechatAvatarUrl
  if (user.verifiedAt === undefined) patch.verifiedAt = data.verifiedAt
  if (Object.keys(patch).length) await collection.doc(user._id).update({ data: patch })
